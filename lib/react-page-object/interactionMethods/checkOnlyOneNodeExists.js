...
export default function checkOnlyOneNodeExists(wrapper, methodName, findWrapperMethodArguments, args) {
  ...
  if (wrapperLength !== 1) {
    ...
    if (wrapperLength === 0) {
      message = `page.${methodName}(${formattedArgs}) failed because no matching React elements were found by page.${findWrapperMethodName}(${formattedFindWrapperMethodArgs})`
    }

    if (wrapperLength > 1) {
      message = `page.${methodName}(${formattedArgs}) failed because ${wrapperLength} matching React elements were found by page.${findWrapperMethodName}(${formattedFindWrapperMethodArgs})`
    }
